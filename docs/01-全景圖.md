...
| embedding 參數量 | \|V\| · d，這裡是 6400 × 256 = 1.6M |
| logits 張量大小 | B × T × \|V\|，訓練時最大的中間張量 |
| 序列長度 | BPE 讓序列比字元級短約 30%，而 attention 是 O(n²)，計算量差更多 |
...
| 層 | 前向 | 反向 |
|---|---|---|
| Linear | y = xWᵀ | ∂L/∂x = ∂L/∂yW，∂L/∂W = (∂L/∂y)ᵀx |
| Embedding | x = Eᵥ（gather） | ∂L/∂Eᵥ = Σₜ : idₜ = v Gₜ（scatter-add） |
| RMSNorm | x̂ = x / r | 1/r(a - x̂mean(a ⊙ x̂)) |
| RoPE | 旋轉 +θ | 旋轉 -θ |
| softmax + CE | p = softmax(z) | ∂L/∂z = p - y |
| 殘差 y = x + f(x) | 分岔 | **相加** |
