...
同樣的 $\eta$、同樣的更新規則。跑 `train.py` 印出的參數表就是證據：
...
$\operatorname{onehot}(v)$ 只有第 $v$ 個元素是 $1$、其餘全 $0$。乘出來就是把 $E$ 的第 $v$ 列抄出來：

$$x_j = \sum_{k=1}^{V} \operatorname{onehot}(v)_k \, E_{kj} = E_{vj}$$

既然 $\dfrac{V-1}{V} = 99.98\%$ 的乘法都是「乘以 $0$」，實作當然直接索引：
...
把整批寫成矩陣形式 $X = O\,E$（$O$ 是常數 one-hot 矩陣），
套用線性層的標準結果：
...
| 沒出現過的 token → 梯度是 $\mathbf{0}$ | $O^{\top}$ 那一列全是 $0$ |
...
輸出側那條有封閉解。因為 $\text{logits}_{t} = \mathbf{h}_t E^{\top}$：
...
注意這裡的點積是「隱藏狀態 $\cdot$ 詞向量」，**不是**「詞向量 $\cdot$ 詞向量」。
...
| 格子 | $\dfrac{L_+ - L_-}{2\varepsilon}$ | 公式 | 說明 |
|---|---:|---:|---|
| $E_{2,0}$（在） | $0.246048$ | $0.246000$ | 查兩次，斜率加倍 |
| $E_{0,0}$（貓） | $0.123024$ | $0.123000$ | 查一次 |
| $E_{1,0}$（狗） | $0.000000$ | $0.000000$ | **沒被查到** |

「狗」那一列 $L(w+\varepsilon)$ 和 $L(w-\varepsilon)$ **完全相等**——
那個數字沒參與計算，動它 $L$ 不會變。

`tests/test_layers.py` 對這一層的驗證結果：相對誤差 $6.80\times10^{-14}$。
