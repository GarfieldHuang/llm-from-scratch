...
同樣的 `lr`、同樣的更新規則。跑 `train.py` 印出的參數表就是證據：
...
`one_hot(id)` 只有一個 1、其餘全 0。乘出來就是把 `E` 的第 `id` 列抄出來。

既然 99.98% 的乘法都是「乘以 0」，實作當然直接索引：

```python
x = E[ids]
```

兩者**結果逐位元相同**，但查表快 20 倍以上、省 8 倍記憶體。
...
| 現象 | 為什麼 |
|---|---|
| 同一個 token 出現 k 次 → 那一列收到 k 筆梯度**相加** | `O^T` 那一列有 k 個 1，矩陣乘法自然就加起來了 |
...
這是最容易混淆的地方。因為 weight tying（`E` 同時是輸出層的權重），
它會收到**兩份**梯度：
...
| 輸出側 | `logits = h @ E^T`，softmax 那條路 | **稠密**，每一步每一列都被碰到 |
...
梯度問的是：「把 `E[4][0]` 動一丁點，`L` 會變多少？」——這個問題跟 id 是不是離散完全無關。

id 只決定「哪些數字參與計算」。沒參與的，斜率算出來自然是 0：
...
`tests/test_layers.py` 用有限差分驗證過這一層，相對誤差 `6.80e-14`。
