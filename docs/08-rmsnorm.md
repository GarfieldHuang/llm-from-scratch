...
| x̂ | 正規化後的向量，長度被拉到約 √n | 否 |
| g | 每個維度一個的縮放係數 | **是**，初始值全為 1 |
| ε | 防止除以零，預設 10⁻⁵ | 否，固定常數 |
...
| r | 0.019711 |
| x̂（正規化後） | +0.83548 -2.41710 +1.27034 +0.23929 |
...
這裡是關鍵——yᵢ 透過**兩條路**依賴 xⱼ：

1. 分子裡的 xᵢ 本身（只有 i = j 時）
2. 分母 r 裡面也含有 xⱼ（每個 j 都有）
...
其中 δᵢⱼ 是 Kronecker delta（i=j 時為 1，否則為 0）。
...
它的作用是**把梯度中沿著 x̂ 方向的分量扣掉**。

理由：x̂ 的長度已經被 r 固定住了（‖x̂‖= √n）。
沿著它自己的方向推，只會改變長度——而長度馬上又會被正規化消掉，
所以那個方向的梯度**是無效的**，扣掉才正確。

幾何上，這是把梯度**投影到與 x̂ 垂直的超平面上**。
...
結果：最大相對誤差 2.13×10⁻⁷，**PASS**。

（這一層的誤差比 Linear 的 10⁻¹² 大，是因為它含有開根號和除法，
浮點運算的累積誤差比較多。10⁻⁷ 對 `float64` 來說仍在合理範圍。）
