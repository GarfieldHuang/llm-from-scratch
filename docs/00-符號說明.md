...
| 符號 | 意思 |
|---|---|
| ∂L/∂x | L 對**單一個數字** x 的偏導數。白話：「把 x 動一丁點，L 會變多少」 |
| ∂L/∂**x** | 對**整個向量**微分（粗體代表向量）。結果跟 **x** 同形狀，每個元素各是一個偏導數 |
| f'(z) | 單變數函數的導數 |
...
| softmax(z)ᵢ | exp(zᵢ) / Σₖ exp(zₖ)，把一排分數變成機率（和為 1） |
| mean(x) | 平均值 |
...
程式碼寫 `x @ W.T`，因為 PyTorch 的慣例是 W 的形狀為 (d_out, d_in)。
