...
因為多了一個矩陣，為了維持總參數量相當，$d_{\text{ff}}$ 通常取 $\frac{8}{3}d$
而不是傳統的 $4d$。
...
| 維度 | $g$（gate） | $s = \operatorname{SiLU}(g)$ | $u$（up） | $m = s \odot u$ |
|---:|---:|---:|---:|---:|
...
看第 3 個維度：$u_3 = -0.03482$ 本身就很小，乘上 $s_3 = 0.198$ 之後
...
最大相對誤差 $1.69\times10^{-6}$，**PASS**。

（這是九層裡誤差最大的一個，因為 SiLU 的導數含有 $\sigma(1-\sigma)$，
連續兩次浮點運算的誤差會累積。$10^{-6}$ 仍在合理範圍。）
