...
## 第一棒：$\dfrac{\partial L}{\partial z} = p - y$

softmax 和 cross-entropy 分開推都很醜（會出現 $V \times V$ 的 Jacobian），
但合起來推會神奇地簡化。
...
| **4** | 0.11176 | **1** | $\mathbf{-0.008882}$ | **拉上來**（正解） |
...
注意 $\partial L/\partial \mathbf{x}$ 用 $W$、$\partial L/\partial W$ 用 $\mathbf{x}$ ——
...
那個孤零零的 $\partial L / \partial \mathbf{y}$ 就是「直達路徑」——梯度可以完全不衰減地穿過去。
**沒有殘差的話，20 層以上基本訓不動。**
...
令 $a_i = \dfrac{\partial L}{\partial y_i}\,g_i$，鏈鎖律加總後化簡：
...
第二項 $-\hat{x}\operatorname{mean}(a\odot\hat{x})$ 的意義是**把沿著 $\hat{x}$ 方向的分量扣掉**——
因為 $\hat{x}$ 的長度已經被 $r$ 固定住了，沿它自己的方向推是無效的。
...
除以 $\sqrt{d_{\text{head}}}$ 的理由：$Q$、$K$ 各維度變異數約為 1，
點積 $d$ 個維度後變異數變成 $d$，標準差 $\sqrt{d}$。
$d = 64$ 時分數會散到 $\pm 8$ 以上，softmax 吃到這種輸入會變得極度尖銳，梯度就消失了。
...
被遮罩的位置 $A_{ij} = 0$，公式裡的 $A_j(\cdots)$ 自動讓梯度也是 0，**不需要另外處理**。
...
反向經過同一個除法時，梯度也跟著放大。
（第二項 $-\hat{x}\operatorname{mean}(a\odot\hat{x})$ 會抵消掉一部分，所以實測倍率比 $1/r$ 略小。）
...
| 層 | 最大相對誤差 | |
|---|---:|---|
| Linear（無 bias） | $2.29\times10^{-12}$ | PASS |
| Linear（有 bias） | $2.03\times10^{-12}$ | PASS |
| RMSNorm | $2.13\times10^{-7}$ | PASS |
| Embedding（scatter-add） | $6.80\times10^{-14}$ | PASS |
| TiedOutputHead | $5.61\times10^{-12}$ | PASS |
| SwiGLU | $1.69\times10^{-6}$ | PASS |
| RoPE | $2.19\times10^{-12}$ | PASS |
| CausalSelfAttention | $3.32\times10^{-7}$ | PASS |
| Softmax + CrossEntropy | $7.50\times10^{-8}$ | PASS |

用**中央差分**（兩邊各推一次）而不是單邊，因為誤差是 $O(\varepsilon^2)$ 而不是 $O(\varepsilon)$。

$\varepsilon$ 也不能太小——`float32` 只有約 7 位有效數字，兩個相近的數相減會被捨入誤差吃掉。
...
真正動手的是 optimizer，而且方向**相反**（往下坡走）、步伐只有 $\text{lr}$ 那麼小。
